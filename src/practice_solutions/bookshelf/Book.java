package practice_solutions.bookshelf;
...
public class Book {
    ...
    public static Boolean isTitleWord(String word) {
        ...
        String nonTitleWords = "a an the and but for nor or so yet at by in into near of on to than via";
        return !nonTitleWords.contains(word.toLowerCase());
    ...
    public void setAuthor(String author) {
        ...
        if (author.isEmpty()) {
            ...
            for (String each : author.split(" ")) {
                fixed += capitalize(each) + " ";
            }

            this.author = fixed.trim();
        }
    }
    ...
    public void setTitle(String title) {
        ...
        if (arr.length == 0) {
            ...
            String fixed = capitalize(arr[0]) + " ";

            for (int i = 1; i < arr.length; i++) {
                if (isTitleWord(arr[i])) {
                    fixed += capitalize(arr[i]) + " ";
                } else {
                    fixed += arr[i].toLowerCase() + " ";
                }
            }
